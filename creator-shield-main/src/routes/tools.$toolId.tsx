...
import { toolPages } from "@/data/seo-pages";
import { ServiceLandingPage } from "@/components/seo/ServiceLandingPage";
import { generateSEOSchemas } from "@/lib/seo";

export const Route = createFileRoute("/tools/$toolId")({
  loader: ({ params }) => {
    const data = toolPages[params.toolId];
    if (!data) throw notFound();
    return { pageData: data, id: params.toolId };
  },
  head: ({ loaderData }) => {
  ...
  component: () => <ServiceLandingPage pageData={Route.useLoaderData().pageData} slug={Route.useLoaderData().id} isToolRoute={true} />,
});
